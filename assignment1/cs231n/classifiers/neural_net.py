...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        exp_prod = np.exp(scores)
        exp_prod_sum = np.sum(exp_prod, axis = 1)
        exp_prod_y = exp_prod[np.arange(N), y]
        exp_loss = exp_prod_y/exp_prod_sum
        loss = - np.sum(np.log(exp_loss)) / N + reg * np.sum(W1*W1) + reg * np.sum(W2*W2)
        ...
        # get dloss/dscore 
        dlds = exp_prod/exp_prod_sum.reshape(N, 1)
        dlds[np.arange(N), y] -= 1 
        dlds /= N 
        # apply chain rule on second layer 
        grads["b2"] = np.sum(dlds, axis = 0)
        grads["W2"] = X1_relu.T.dot(dlds) + 2 * reg * W2 
        f = np.vectorize(lambda x: 1 if x > 0 else 0)
        mask = f(X1)
        dldx1 = dlds.dot(W2.T) * mask
        # apply chain rule on the first layer  
        grads["b1"] = np.sum(dldx1, axis = 0)
        grads["W1"] = X.T.dot(dldx1) + 2 * reg * W1 
